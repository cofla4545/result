...
# InsightFace 모델 로드
app = FaceAnalysis(providers=['CPUExecutionProvider'])  # CUDAExecutionProvider 제거
...
# 이 부분을 제거하거나 주석 처리하여 비디오 처리와 결합이 자동으로 실행되지 않도록 합니다.
# 학습할 얼굴 이미지 경로 리스트
...
#                 roi = frame[y_min:y_max, x_min:x_max].copy()  # 복사본 생성
#                 roi_small = cv2.resize(roi, (16, 16), interpolation=cv2.INTER_LINEAR)
#                 roi_mosaic = cv2.resize(roi_small, (xmax - xmin, ymax - ymin), interpolation=cv2.INTER_NEAREST)
