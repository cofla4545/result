...
# InsightFace 모델 로드
face_app = FaceAnalysis(providers=['CPUExecutionProvider'])  # CUDAExecutionProvider 제거
...
def get_face_embeddings(image_paths):
    ...
    if not embeddings:
        print("Warning: No faces detected in the provided images.")
...
# 파일이 존재하지 않으면 빈 배열을 사용
if os.path.exists(learning_image_paths[0]):
    learning_embeddings = get_face_embeddings(learning_image_paths)
else:
    print(f"Warning: Learning image not found at {learning_image_paths[0]}. Proceeding without it.")
    learning_embeddings = np.array([])  # 빈 배열 사용
